...
while dif>tol && nIter<iterMax
    ...
    H = (1/2) .* (1+(2/pi) .* atan(phi ./ epHeaviside));
    ...
    %Vese segmentation
    A = mu ./ sqrt(nu^2 + (phi_iFwd .* phi_extend).^2 + (phi_jcent .* phi_extend).^2); %TODO 13: Line to complete
    B = mu ./ sqrt(nu^2 + (phi_icent .* phi_extend).^2 + (phi_jFwd .* phi_extend).^2); %TODO 14: Line to complete
    ...
    phi = (phi + dt .* delta_phi .* (A1 + A2 + B1 + B2 - nu - c1Factor + c2Factor) ) ./ (1 + dt * delta_phi .* divisionFactor); %TODO 15: Line to complete
